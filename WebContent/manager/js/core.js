...
var bodymenudata ={
	"sorticon":{
		"name":"排列图标",
		"items":{
			"subSortName":{"name":"名称", "icon":"edit", callback:function(){alert("名称");}},
				"subSortBig":{"name":"大小", "icon":"cut", callback:function(){alert("大小");}},
					"subSortType":{"name":"类型", "icon":"quit", callback:function(){alert("类型");}}
					}
				},
				"attr":{"name":"属性","icon":"quit",callback:function(){
					alert("属性"+$(this).text());
				}}
			};
var iconRightMenu = {
	"open":{"name":"打开","icon":"edit",callback:function(){Core.create($(this));}},
		"attr":{"name":"属性","icon":"cut",callback:function(){alert("属性"+$(this).text());}}
		};
var taskbarrightmenu ={
	"restore":{"name":"最小化", "icon":"cut", callback:function(){
		$('#window_'+$(this).attr('window')+'_inner .title-handle .ha-min').click();
	}},
	"close":{"name":"关闭窗口","icon":"edit",callback:function(){
		$('#window_'+$(this).attr('window')+'_inner .title-handle .ha-close').click();
		$('.task-menu').hide();
	}}
};
var taskbarmenu ={
	"minAllWindow":{"name":"最小化所有窗口", "icon":"cut", callback:function(){Core.showDesktop();}},
	"sorticon":{"name":"属性","icon":"edit",callback:function(){alert("属性");}}
	};
var iconClick = {
	"sorticon":{"name":"打开","icon":"edit",callback:function(){alert("open");}}
	};
//桌面右键菜单
Core.init = function(){
...
//	});
	...
	$(window).on('load',function(){
		...
		//自动启动的个人信息窗口
		//Core.createPersonInfoWindow(110);
	});
	...
	//绑定任务栏点击事件
	$('.task-window').on('click','li',function(){
		Core.taskwindow($(this));
	});
	//    $('.task-window').on('contextmenu',function(){
	//        //展示自定义右键菜单
	...
	//    });
	//任务栏窗口图标右键菜单
	$(".task-window").contextMenu({
		selector:	"li",
		autoHide: true,
		items: taskbarrightmenu
	});
	//任务栏右键菜单
	$.contextMenu({
	...
	$.contextMenu.types.label = function(item,opt,root){
		...
		items: {
			...
			"systemConfig":{name:"\u66f4\u65b0\u65e5\u5fd7"/*更新日志*/, icon:"edit", callback: function(key,opt){
//				$("#systemConfig").dialog("open").dialogExtend(getdialogExtendOptions());
//				$("#systemConfig").dialogExtend("restore");
				Core.createNewWindow(111,"更新日志",getBasePath()+"/updateLog.do",380,400,true );
			}},
			"personInfo":{name:"个人信息", icon:"cut", callback:function(){
					Core.createNewWindow(110,"个人信息",getBasePath()+"/manager/user/personInfo.do",380,400,true );
			}},
			"sep1":"----------------",
			"shutdown":{name:"\u6CE8\u9500\u767B\u9646"/*注销登陆*/,icon:"cut",callback: function(key,opt){
				location.href=getBasePath()+"/user/doLogout.do";
			}}
		},
	});
	...
	//绑定窗口点击事件
	$('body').on('click','.window-container',function(){
		Core.container($(this));
	});
	//系统设置处理
	$('.task-panel li').on('click',function(){
		...
		//开始菜单鼠标经过效果
	$("#taskBar_startBtn").hover(function(){
	...
	//自动启动的窗口
	//Core.createPersonInfoWindow();
};
//创建个人信息固定窗口
Core.createNewWindow = function(int_windowNum,t,www,w,h){
		opt = {num:int_windowNum, title:t,url:www,width:w,height:h,resize:true};
...
Core.create = function(obj,opt){
	...
	if(typeof(obj)==='string'){
		var options = {
			num		:obj,//Date.parse(new Date()),
			imgsrc	:"http://api.jq-school.com/images/shortcut/news.png",
			title	:opt.title,
			url		:opt.url,
			width	:opt.width,
			height	:opt.height,
...
Core.handle = function(obj){
	...
	//刷新
	obj.find("#refresh").on("click",function(e){
...
Core.bindWindowMove = function(obj){
	obj.find(".title-bar").mousedown(function(e){
		//改变窗口为选中样式
		$('.window-container').removeClass('window-current');
		obj.addClass('window-current').css({
			'z-index':Core.config.createIndexid
		...
		//增加背景遮罩层
		_cache.MoveLayOut = GetLayOutBox();
		var lay = (!$.support.leadingWhitespace)? _cache.MoveLayOut : $(window);
		//绑定鼠标移动事件
		lay.mousemove(function(e){
			_cache.MoveLayOut.show();
...
Core.bindWindowResize = function(obj){
	...
	function bindResize(r){
		obj.find("div[resize='"+r+"']").mousedown(function(e){
			//增加背景遮罩层
			_cache.MoveLayOut = GetLayOutBox();
			var lay = (!$.support.leadingWhitespace)? _cache.MoveLayOut : $(window);
			cy = e.clientY;
